...
While the objective of Replicating NIMBY is to analyze the relationship between crime cannabis dispensaries location using the data sources presented in the original paper's methodology, I had to adjust my approach due to data quality and time constraints. The diagram below shows how I used R code to build the linear model and it's variables. I broke the code down into four parts which are highlighted in this site's navigation bar.
...
In summation, the data mining pipeline consisted of several stages.
...
-   Demographic data was extracted using the TidyCensus package, which facilitated the retrieval of data for specific variables from the American Community Survey (ACS) census dataset .\| [code](https://brookemwalters.github.io/Nimby_Site/Label%20the%20Dispensaries%20with%20GeoCodes_step2.html "Geocodes, Census Variables, and Dispensaries Per Capita")

-   The datasets were then joined together by county-level identifiers (GEOID), allowing for the computation of per capita values for dispensaries and crime rates and year-over-year fluctuations. \| [code](https://brookemwalters.github.io/Nimby_Site/calculate%20crime%20and%20dispensaries%20per%20capita_step3.html "per capita")
