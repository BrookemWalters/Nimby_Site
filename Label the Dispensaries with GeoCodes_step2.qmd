...
table2 <- combined_table %>%gt() %>% 
  ...
  tab_style(
    ...
    opt_table_lines(extent =  "default") %>% # keep table lines
  tab_options(
    column_labels.border.top.color = "white",
    column_labels.border.top.width = px(3), # adjusts space between title and column headers
    column_labels.border.bottom.color = "black",
    table_body.border.bottom.color =  "black",
      table.width = pct(100),
    data_row.padding = px(10),
    column_labels.background.color = "#EAB8B7") %>% 
  
  # align text to the center
  tab_style(style = cell_text(
    align = "center"),
    locations = cells_body(columns = TRUE) # Apply the style to all columns in the body of the table
  ) %>% 
  tab_style(
    style = list(
      cell_fill(color = "black", alpha = 0.2)),
    locations = cells_body(
      columns = vars(`Colorado Average`)))
