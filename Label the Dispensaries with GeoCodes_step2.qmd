...
3.  retrieves demographic populations\
...
This section of my methodology was the most exciting for me, yet it was also where I recognized the need to adapt my approach. My first limitation encountered was the challenge of matching dispensary locations with their corresponding census tract locations. To achieve this, I would need to obtain each location's physical address, which was not provided by Colorado.gov's dispensary archive. While this information can be accessed through vendors or searching the web, I proceed with county-level data for the scope of this project.

Nonetheless, the TIGRIS package's ability to create maps at the census tract level remains a potent instrument in an analyst's arsenal. Although the tract level identifiers are not utilized beyond this point, I have included it as a reference for future projects and as part of my Replicating NIMBY journey. Owing to the differences in methodology, I do not anticipate my figures to align flawlessly; however, examining the overall trends for congruence remains a worthwhile endeavor.
...
At this stage, the zip codes are my main geographical identifier for my dispensary data frame. In an attempt to find the tract level data, I tapped into the tigris package. It was here I realized that a county level analysis was my best approach. My original goal was to pull in the spacial data for mapping later.
...
Here I join two datasets using "GEOID", aggregate and sums up the annual population and demographic factors by geographic ID, and creates new columns that represent the total population and economic indicators for each geographic ID. The resulting dataset has one row per unique geographic ID.
...
Prepare the data to be presented in a simple summarized table
...
format the table for a side by side comparison [*adapted from "[Top 7 Packages for Making Beautiful Tables in R](https://towardsdatascience.com/top-7-packages-for-making-beautiful-tables-in-r-7683d054e541 "Devashree Madhugiri")"*]{style="font-size: 80%;"}

```{r}

# use the gt package to format the intel
combined_table %>%gt() %>% 
    sub_missing(
    columns = everything(), # remove the null vaules from the colorado average data
    missing_text = ""
  ) %>% 
  
  # format the title
  tab_header(title = "average change in dispensary density by local characteristics",
             subtitle = "Brooke's Version"
              ) %>%
  
  # format the first column
  tab_style(style = list(cell_fill(color = "#DEDDB6"), 
                         cell_text(weight = "bold")),
            locations = cells_body(columns = Quartile)) %>% 
  
  # format the state comparison
  tab_style(style = list(cell_fill(color = "#EAB8B7"), 
                         cell_text(weight = "bold")),
