...
aes(x =Longitude, y =Latitude, fill = crime_per_cap))
...
county_centers <-  read.csv("~/gvsu/winter 23/CIS 635/NIMBY/cleaning/geo_files/co_cords.csv") # info downloaded from the web

counties <- map_data("county", "colorado")
counties <- rename(counties, County = subregion)

# link the average dispensary county with the center of the county
dsp_points <- left_join(county_centers, linear_variables, by = "County")



#link the crime data with the shape files
...
my_map <- ggplot() +
  # heat map
  geom_polygon(data = my_map_data , 
               aes(x = long, y = lat, group = group, fill = crime_per_cap, color = "")) +
 labs(title = "Crime Rate and Dispensary Count by County in Colorado",
      ...
      caption = "Mineral county crime data not available",
       fill = "crimes per 10k pop",
      color = "county boarder") +
  scale_fill_gradient(low = "#DAF0FA", high = "#B41876"
                      ) +
    scale_color_manual(values = "black", guide = FALSE) + # Remove border color from legend
  # Bubble chart with updated label
 
  # geo bubble charts
  geom_point(data = dsp_points, aes(x = Longitude, y = Latitude, size = dsps_average),
             color = "#23CB7F", 
            stroke_width = 1,
             alpha = 0.8)+
  labs(size = "no. of dispensaries")+

  theme_map() +
   theme(plot.caption = element_text(size = 8, face = "italic",
                                     hjust = .1))
# Remove border color from legend
  # guides(color = guide_legend(override.aes = list(color = NA)))

my_map
ggsave("images/co_map.png", plot = my_map)
```
